...
void SLPA::GLPA_asyn_pointer_omp_v1(){
	//pointer version:
	//	 store the pointer of nb in *nbList_P*
	//   save time for retrieving hashTable
	time_t st=time(NULL);
...
void SLPA::GLPA_asyn_pointer_omp_v2(){
	...
	for(int t=1;t<maxT;t++){
		...
		// {
			...
			#pragma omp parallel for schedule(dynamic) shared(nbWs) num_threads(numThreads)
			for(int i=0;i<net->N;i++)
			{
				NODE *v, *nbv;
				int id = omp_get_thread_num();
				...
				//b.select one of the most frequent label
				// label=ceateHistogram_selRandMax(nbWs);
				int label = ceateHistogram_selRandMax_qiao_v1(nbWs[id]);
				// cout << "hello3" << endl;
				//c. update the WQ **IMMEDIATELY**
				v->WQueue.push_back(label);
			}
...
void SLPA::GLPA_asyn_pointer_omp_v7(){
	//pointer version:
	//	 store the pointer of nb in *nbList_P*
	//   save time for retrieving hashTable
	time_t st=time(NULL);
	...
	// cout<<endl;
	// cout<<"Iteration is over (takes "<<difftime(time(NULL),st)<< " seconds)"<<endl;
} // end of SLPA::GLPA_asyn_pointer_omp_v7()
...
void SLPA::GLPA_asyn_pointer_qiao_v3(){
	//pointer version:
	//	 store the pointer of nb in *nbList_P*
	//   save time for retrieving hashTable
	time_t st=time(NULL);
	...
	// cout<<endl;
	// cout<<"Iteration is over (takes "<<difftime(time(NULL),st)<< " seconds)"<<endl;
} // end of SLPA::GLPA_asyn_pointer_qiao_v3()

void SLPA::GLPA_asyn_pointer_qiao_v4(){
	//pointer version:
	//	 store the pointer of nb in *nbList_P*
	//   save time for retrieving hashTable
	time_t st=time(NULL);
...
void SLPA::GLPA_asyn_pointer_qiao_v6(){
	//pointer version:
	//	 store the pointer of nb in *nbList_P*
	//   save time for retrieving hashTable
	time_t st=time(NULL);
	...
	#pragma omp parallel num_threads(numThreads) shared(labels_h)
	{
		...
		map<int, int>::iterator mit;
		vector<int> mostLabelsList_s;
		vector<pair<int, int>> wordsList_s;
		...
		for(int t=1;t<maxT;t++)
		{
			...
			for(int i=0;i<net->N;i++)
			{
				...
				//b.select one of the most frequent label
				// label=ceateHistogram_selRandMax(nbWs);
				labels_h[i] = selectMostFrequentLabel_v5(nbWs_s, mtrand1_s, wordsList_s, mostLabelsList_s);
	...
	// cout<<endl;
	// cout<<"Iteration is over (takes "<<difftime(time(NULL),st)<< " seconds)"<<endl;
} // end of SLPA::GLPA_asyn_pointer_qiao_v6()
...
int SLPA::selectMostFrequentLabel_v5(map<int, int>& labelsList, MTRand& mtrand1_s, vector<pair<int, int>>& wordsList, vector<int>& mostLabelsList)
{
	...
	for (mit = labelsList.begin(); mit != labelsList.end(); mit ++)
	{
		wordsList.push_back(*mit);
	}

	for (int i = 0; i < wordsList.size(); i ++)
	{
		if (wordsList[i].second > maximum)
		{
	...
	return label;
} // SLPA::selectMostFrequentLabel_v5
